...
// 计算属性
const layers = computed(() => projectStore.currentDrawing.value?.layers || []);
const currentLayer = computed(() => projectStore.currentDrawing.value?.currentLayer || '0');
...
function setCurrentLayer(name: string) {
  if (projectStore.currentDrawing.value) {
    projectStore.currentDrawing.value.currentLayer = name;
...
function addLayer() {
  ...
  projectStore.currentDrawing.value?.layers.push(newLayer);
  projectStore.markDirty();
...
function refreshLayers() {
  // 重新统计实体数量
  if (projectStore.currentDrawing.value) {
    const entities = projectStore.currentDrawing.value.entities || [];
    for (const layer of projectStore.currentDrawing.value.layers) {
...
function mergeToCurrent(layer: any) {
  ...
  layers.value = layers.value.filter(l => l.name !== layer.name);
  projectStore.markDirty();
}
