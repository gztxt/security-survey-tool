...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle('fs:writeFile', async (_, path: string, content: string) => {
    try {
      writeFileSync(assertAllowedPath(path), content, 'utf8');
      return true;
    } catch {
      return false;
    }
...
async function convertDwgToDxf(dwgPath: string) {
  return new Promise((resolve, reject) => {
    const odaPath = getOdaConverterPath();
    if (!existsSync(odaPath)) {
      reject(new Error('ODA File Converter not found'));
      return;
    }
    ...
    proc.on('close', (code) => {
      if (code === 0 && existsSync(dxfPath)) {
        resolve({ success: true, dxfPath });
      } else {
        reject(new Error(`ODA conversion failed with code ${code}`));
      }
    });
...
// 导出功能存根（尚未实现，明确向上层报错，不再假装成功）
async function exportPointMap(options: any) { throw new Error('导出功能尚未实现'); }
async function exportFovMap(options: any) { throw new Error('导出功能尚未实现'); }
async function exportTopology(options: any) { throw new Error('导出功能尚未实现'); }
async function exportDeviceList(options: any) { throw new Error('导出功能尚未实现'); }
async function exportCableSchedule(options: any) { throw new Error('导出功能尚未实现'); }
async function exportReport(options: any) { throw new Error('导出功能尚未实现'); }

function getDeviceLibrary() { throw new Error('设备库功能尚未实现'); }
function addCustomDevice(device: any) { throw new Error('设备库功能尚未实现'); }
function importDeviceLibrary(json: string) { throw new Error('设备库功能尚未实现'); }
function exportDeviceLibrary() { throw new Error('设备库功能尚未实现'); }
