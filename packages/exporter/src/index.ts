...
export class Exporter {
  ...
  private exportDeviceListPdf(data: any): ExportFile {
    ...
    for (const cat of data.summary) {
      doc.fontSize(12).text(`${cat.category} (共 ${cat.count} 台)`, { bold: true });
    ...
    doc.moveDown();
    doc.fontSize(12).text(`总计: ${data.totalDevices} 台设备，预估成本: ¥${data.totalCost}`, { bold: true });
  ...
  private exportCableScheduleXlsx(data: any): ExportFile {
    ...
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
    return { path: 'cable_schedule.xlsx', format: 'xlsx', size: buf.length };
  }
  ...
  private exportCableScheduleCsv(data: any): ExportFile {
    ...
    return { path: 'cable_schedule.csv', format: 'csv', size: Buffer.byteLength(rows.join('\n'), 'utf8') };
  }

  private exportCableSchedulePdf(data: any): ExportFile {
    const doc = new PDFKit({ size: 'A4', margin: 40 });
    const chunks: Buffer[] = [];
    ...
    doc.fontSize(12).text(`总长度: ${data.totalLength.toFixed(1)} 米`, { bold: true });
    doc.moveDown();
...
<body>
  ...
  <div class="page">
    ...
    <table>
      ...
      <tbody>
        ...
        {{/each}}
        <tr class="header-row"><td>合计</td><td>{{cableSchedule.totalLength}}</td><td>{{cableSchedule.byType.reduce((s,t)=>s+t.count,0)}}</td><td></td></tr>
      </tbody>
    </table>
    <h4>分楼层明细</h4>
  ...
  <div class="page">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>{{deviceId}}</td><td>{{power}}</td><td>{{voltage}}</td><td>{{distance}}</td><td>{{drop.toFixed(2)}}</td></tr>
        {{/each}}
      </tbody>
    </table>
  </div>
  ...
  private async htmlToPdf(html: string): Promise<Buffer> {
    // 使用 pdf-lib 或 puppeteer 转换
    // 这里简化：返回空 buffer，实际需集成 puppeteer/chromium
    return Buffer.from('');
  }
  ...
  private canvasToPdf(canvas: HTMLCanvasElement, outputPath: string, filename: string, bounds: BBox): ExportFile {
    // 使用 pdf-lib 将 canvas 转为 PDF
    return { path: outputPath, format: 'pdf', size: 0, drawingId: '' };
  }
  ...
  private canvasToImage(canvas: HTMLCanvasElement, outputPath: string, filename: string, format: ExportFormat): ExportFile {
    ...
    return { path: outputPath, format, size: buf.length, drawingId: '' };
  }
  ...
  private createZipPackage(files: ExportFile[], outputDir: string): Promise<ExportFile> {
    return new Promise((resolve) => {
      ...
      for (const file of files) {
        zip.file(file.path, Buffer.alloc(file.size)); // 简化
      }
      zip.generateAsync({ type: 'nodebuffer' }).then((content: Buffer) => {
        resolve({ path: `${this.sanitizeFilename(this.project.name)}_导出包.zip`, format: 'zip', size: content.length });
      });
    });
  }
