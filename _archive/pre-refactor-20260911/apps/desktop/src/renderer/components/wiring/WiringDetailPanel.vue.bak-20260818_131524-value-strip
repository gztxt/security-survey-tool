...
const startDevice = computed(() => {
  ...
  return projectStore.projectDevices.value.find(d => d.id === cable.value?.startDeviceId) || null;
});

const endDevice = computed(() => {
  if (!cable.value) return null;
  return projectStore.projectDevices.value.find(d => d.id === cable.value?.endDeviceId) || null;
});
