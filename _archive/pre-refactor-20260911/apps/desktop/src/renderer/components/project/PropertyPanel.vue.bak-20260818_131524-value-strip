...
const device = computed(() => {
  ...
  return projectStore.projectDevices.value.find(d => d.id === props.deviceId) || null;
});

const cable = computed(() => {
  if (!props.cableId) return null;
  return projectStore.projectCables.value.find(c => c.id === props.cableId) || null;
});

const compatibleModels = computed(() => {
  if (!device.value) return [];
  const currentModel = deviceLibraryStore.allDevices.value.find(d => d.id === device.value?.modelId);
  if (!currentModel) return deviceLibraryStore.allDevices.value;
  return deviceLibraryStore.allDevices.value.filter(d =>
...
function updateCustomSpec(key: string, event: Event) {
  if (!device.value?.customSpecs) return;
  const value = (event.target as HTMLInputElement).value;
  device.value.customSpecs[key] = value;
  updateDevice();
}
...
function deleteCustomSpec(key: string) {
  if (!device.value?.customSpecs) return;
  delete device.value.customSpecs[key];
  updateDevice();
}
