...
function onMouseMove(e: MouseEvent) {
  ...
  if (isPanning.value) {
    ...
    if (Math.hypot(modelPos.x - lastPoint.x, modelPos.y - lastPoint.y) > 5) {
      tempWirePath.value.push(modelPos);
      renderOverlay();
    }
  } else if (tool.value === 'tray' && tempTrayPath.value.length > 0) {
    const lastPoint = tempTrayPath.value[tempTrayPath.value.length - 1];
    if (Math.hypot(modelPos.x - lastPoint.x, modelPos.y - lastPoint.y) > 5) {
      tempTrayPath.value.push(modelPos);
      renderOverlay();
    }
...
function placeDevice(modelPos: Point2D) {
  ...
  const model = deviceLibraryStore.allDevices.value.find(d => d.id === deviceLibraryStore.selectedDeviceId);
  if (!model) return;
